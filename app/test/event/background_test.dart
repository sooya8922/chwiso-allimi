...
import 'package:yeollim_allim/event/models/event.dart';
import 'package:yeollim_allim/event/services/background.dart';
...
void main() {
  ...
  test('첫 실행 — 발송 없이 기준선(notified 셋)만 저장', () async {
    ...
    final stored = (json.decode(sp.getString('notified_keys_event_v1')!) as List).toSet();
    expect(stored, {'new_seoul:n1'}); // 발송은 없었지만 '본 것'으로 마킹
  });

  test('60초 내 재실행 — 즉시알림 파트 스킵(이중 발송 가드)', () async {
    SharedPreferences.setMockInitialValues({
      'notify_run_ts_event_v1': DateTime.now().millisecondsSinceEpoch, // 방금 실행된 것으로
      // notified가 이미 있으므로(빈 목록) 재실행이면 새 행사 발송을 시도할 상황
      'notified_keys_event_v1': '[]',
    });
    // 가드가 스킵하므로 플랫폼 알림 호출까지 안 가서 테스트 환경에서도 예외가 없어야 한다
    await refreshAndNotify(preloaded: feedWithNew(), now: DateTime(2026, 7, 15, 10));
    final sp = await SharedPreferences.getInstance();
    expect(sp.getString('notified_keys_event_v1'), '[]'); // 마킹도 안 됨(파트 전체 스킵)
  });

  test('조용시간(밤 11시) — 발송/저장 모두 미룸 (기존 사용자)', () async {
    SharedPreferences.setMockInitialValues({'notified_keys_event_v1': '[]'});
    await refreshAndNotify(preloaded: feedWithNew(), now: DateTime(2026, 7, 15, 23, 0));
    final sp = await SharedPreferences.getInstance();
    expect(sp.getString('notified_keys_event_v1'), '[]'); // 아침에 알리도록 '안 본' 상태 유지
  });
  ...
  test('조용시간이라도 첫 실행 기준선은 저장 (밤 설치 → 아침 폭탄 방지)', () async {
    ...
    await refreshAndNotify(preloaded: feedWithNew(), now: DateTime(2026, 7, 15, 23, 0));
    final sp = await SharedPreferences.getInstance();
    expect(sp.getString('notified_keys_event_v1'), isNotNull);
  });

  test('손상된 notified 저장소 — self-heal(빈 셋 폴백, 예외 없음)', () async {
    SharedPreferences.setMockInitialValues({'notified_keys_event_v1': '{broken'});
    ...
    SharedPreferences.setMockInitialValues({
      'notified_keys_event_v1': '{broken',
      'subscription_event_v1': '{"areas":[],"kidOnly":true,"freeOnly":true,"keywords":[]}',
    });
    final feed = Feed(
      version: 1, generatedAt: '', sources: const {},
      events: const [
